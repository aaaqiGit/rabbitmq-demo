...
public class DirectRabbitConfig implements BeanPostProcessor {
    ...
    @Bean
    public TopicExchange rabbitmqDemoTopicOrgExchange() {
        //配置orgExchange交换机
        return new TopicExchange(RabbitMQConfig.TOPIC_ORG_EXCHANGE, true, false);
    }

    @Bean
    public Queue orgQueue(){
        return new Queue(RabbitMQConfig.TOPIC_ORGQUEUE, true, false, false);
    }

    @Bean
    public Binding bindTopicOrgExchange() {
        //队列orgQueue绑定到rabbitmqDemoTopicOrgExchange 交换机
        return BindingBuilder.bind(orgQueue())
                .to(rabbitmqDemoTopicOrgExchange())
                .with("org.*");
    }
